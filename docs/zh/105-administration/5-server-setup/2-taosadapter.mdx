...
是为 业务集群提供 RESTful 和 Websocket 访问接口的服务。taosadapter 无需独立安装。在安装了 TDengine server 之后，系统中就已经具备了 taosadapter。但如果想在不同的服务器上分别部署 TDengine 集群 (taosd 组件) 和 taosadapter，则需要在这些服务器上都安装 [TDengine server 安装包](../../server-installation)。关于 taosadapter 的配置和使用细节，请参考 [taosadapter](../../../tdengine-programs/taosadapter)。
