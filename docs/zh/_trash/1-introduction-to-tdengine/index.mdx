...
1. 写入数据，支持
   - [SQL 写入](./next/development/writing-data/SQL-insertion)
   - [无模式（Schemaless）写入](./next/development/writing-data/schemless-writing/overview)，支持多种标准写入协议
     - [InfluxDB Line 协议](./next/development/writing-data/schemless-writing/influxdb-line-protocol)
     - [OpenTSDB Telnet 协议](./next/development/writing-data/schemless-writing/opentsdb-telnet)
     - [OpenTSDB JSON 协议](./next/development/writing-data/schemless-writing/opentsdb-json)
   - 与多种第三方工具的无缝集成，它们都可以仅通过配置而无需任何代码即可将数据写入 TDengine
     - [Telegraf](./next/third-party-connection/telegraf)
     - [Prometheus](./next/third-party-connection/prometheus)
     - [StatsD](./next/third-party-connection/statsd)
     - [collectd](./next/third-party-connection/collectd)
     - [Icinga2](./next/third-party-connection/icinga2)
     - [TCollector](./next/third-party-connection/tcollector)
     - [EMQX](./next/third-party-connection/emqx-broker)
     - [HiveMQ](./next/third-party-connection/hive-mq-broker)
2. 查询数据，支持
   - [标准 SQL](./next/sql-language)，含嵌套查询
   - [时序数据特色函数](./next/sql-language/function#time_series_analytic_functions)
   - [时序数据特色查询](./next/sql-language/query/select)，例如降采样、插值、累加和、时间加权平均、状态窗口、会话窗口等
   - [用户自定义函数（UDF）](./next/development/user-defined-function)
3. [缓存](./next/introduction/cache)，将每张表的最后一条记录缓存起来，这样无需 Redis 就能对时序数据进行高效处理
4. [流式计算（Stream Processing）](./next/sql-language/tmq)，TDengine 不仅支持连续查询，还支持事件驱动的流式计算，这样在处理时序数据时就无需 Flink 或 Spark 这样流式计算组件
5. [数据订阅](./next/development/subscription)，应用程序可以订阅一张表或一组表的数据，提供与 Kafka 相同的 API，而且可以指定过滤条件
6. 可视化
   - 支持与 [Grafana](./next/third-party-connection/grafana) 的无缝集成
   - 支持与 [Google Data Studio](./next/third-party-connection/google-data-studio) 的无缝集成
7. 集群
   - [集群部署](./next/administration/server-setup-and-management)，可以通过增加节点进行水平扩展以提升处理能力
   - 可以通过 [Docker、Kubernetes 部署](./next/deployment-in-cloud)
   - 通过多副本提供高可用能力
8. 管理
   - [监控](./next/administration/monitoring-diagnoses-recovery/monitoring)运行中的 TDengine 实例
   - 多种[数据导入/导出](./next/administration/data-migration)方式
9. 工具
   - 提供[交互式命令行程序（CLI）](./next/tdengine-programs/taos)，便于管理集群，检查系统状态，做即席查询
   - 提供压力测试工具 [taosBenchmark](./next/tdengine-programs/taosbenchmark)，用于测试 TDengine 的性能
10. 编程
    - 提供各种语言的[连接器（Connector）](./next/api-and-connnectors/overview): 如 [C/C++](./next/api-and-connnectors/cpp)、[Java](./next/api-and-connnectors/java)、[Go](./next/api-and-connnectors/go)、[Node.js](./next/api-and-connnectors/node)、[Rust](./next/api-and-connnectors/rust)、[Python](./next/api-and-connnectors/python)、[C#](./next/api-and-connnectors/csharp) 等
    - 支持 [REST 接口](./next/api-and-connnectors/rest-api/)
