...
安装完成后，请根据 [部署指南](../../administration/deployment/server-setup) 配置集群。
...
由于物联网数据的特殊性，数据产生的频率固定以后，TDengine 写入的过程对于（计算和存储）资源消耗都保持一个相对固定的量。《[TDengine 运维指南](../../administration/)》上的描述，该系统中每秒 22000 个写入，消耗 CPU 不到 1 个核。
