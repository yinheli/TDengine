...
class AutoGen:
    ...
    # fill one data
    def fill_fix_data(self, i, marr, data, ignoreType=False):
        datas = ""
        for c in marr:
            if datas != "":
                datas += ","
    ...
    def create_stable(self, stbname, tag_cnt, column_cnt, binary_len, nchar_len):
        ...
        tdLog.info("create_stable sql:" + sql)
        tdSql.execute(sql)

    # create child table
    def create_child(self, stbname, prename, cnt, fill_data='', ignoreType=False):
        self.child_cnt = cnt
        self.child_name = prename
        for i in range(cnt):
            if len(fill_data) > 0:
                tags_data = self.fill_fix_data(i, self.mtags, fill_data, ignoreType)
            else:
                tags_data = self.gen_data(i, self.mtags)

            sql = f"create table {self.dbname}.{prename}{i} using {self.dbname}.{stbname} tags({tags_data})"
            tdLog.info("create_child sql:" + sql)
            tdSql.execute(sql)

        tdLog.info(f"create child tables {cnt} ok")

    def insert_data_child(self, child_name, cnt, batch_size, step, fill_data='', tags_data=''):
        values = ""
        print("insert child data-->" + fill_data + "-->" + tags_data)
        ts = self.ts
        ...
        for i in range(cnt):
            ...
            if len(fill_data) > 0:
                value = self.fill_fix_data(i, self.mcols, fill_data)
            else:
            ...
            if batch_size == 1 or (i > 0 and i % batch_size == 0):
                if len(tags_data) > 0:
                    sql = f"insert into {self.dbname}.{child_name} using {self.dbname}.{self.stbname} tags({tags_data}) values {values}"
                else:
                    sql = f"insert into {self.dbname}.{child_name} values {values}"
                tdLog.debug("insert_data_child sql:" + sql)
                tdSql.execute(sql)
                values = ""
    ...
    def insert_data(self, cnt, bContinue=False, fill_data=''):
        if not bContinue:
            self.ts = 1600000000000
        ...
        for i in range(self.child_cnt):
            ...
            currTs = self.insert_data_child(name, cnt, self.batch_size, 1, fill_data)

        self.ts = currTs
        tdLog.info(f" insert data ok, child table={self.child_cnt} insert rows={cnt}")

    def insert_data_auto_create(self, cnt, child_cnt, child_name, bContinue=False, fill_data='', fill_tag='',
                                ignoreType=False):
        if not bContinue:
            self.ts = 1600000000000
        ...
        for i in range(child_cnt):
            name = f"{child_name}{i}"
            if len(fill_tag) > 0:
                tags_data = self.fill_fix_data(i, self.mtags, fill_tag, ignoreType)
            else:
                tags_data = self.gen_data(i, self.mtags)

            currTs = self.insert_data_child(name, cnt, self.batch_size, 1, fill_data, tags_data)
