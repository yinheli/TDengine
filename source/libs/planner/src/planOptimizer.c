...
static int32_t pushDownCondOptPartJoinOnCondLogicCond(SJoinLogicNode* pJoin, SNode** ppPrimKeyEqCond,
                                                      SNode** ppOnCond) {
...
static bool stbJoinOptShouldBeOptimized(SLogicNode* pNode) {
  ...
  SJoinLogicNode* pJoin = (SJoinLogicNode*)pNode;
  if (pJoin->isSingleTableJoin || NULL == pJoin->pTagEqCond || NULL != pJoin->pTagOnCond ||
      pNode->pChildren->length != 2 || pJoin->hasSubQuery || pJoin->joinAlgo != JOIN_ALGO_UNKNOWN ||
